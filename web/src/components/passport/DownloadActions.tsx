...
import { FileDown, Image as ImageIcon, Printer, FileText, Loader2, AlertCircle, X } from 'lucide-react';
...
export const DownloadActions: React.FC<DownloadActionsProps> = ({
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      setDownloadError(err?.message || 'Failed to download PDF. Please ensure the Photo Engine backend is running.');
    } finally {
  ...
  const handleDownloadSheetImage = async () => {
    ...
    try {
      ...
      setDownloadError(err?.message || 'Failed to download print sheet image. Please verify photo engine connection.');
    } finally {
